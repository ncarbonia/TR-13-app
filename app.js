...
function buildLayout() {
  ...
  if (columns < 2 || measuredStationDistance <= 0) {
    summary.textContent = "Columns per side must be 2+ and measured station distance must be greater than 0.";
    return;
  }
  ...
  sideMeasurements.innerHTML = "<legend>Side elevation measurements from Baseline (start at 0 ft, then measured station distance)</legend>";

  sides.forEach(side => {
    for (let segment = 1; segment < columns; segment += 1) {
      const designDistance = Number(document.getElementById(`${side.key}_design_distance_${segment}`)?.value ?? 60);
  ...
  for (let segment = 1; segment < columns; segment += 1) {
    const sideADesignDistance = Number(document.getElementById(`sideA_design_distance_${segment}`)?.value ?? 60);
    const sideBDesignDistance = Number(document.getElementById(`sideB_design_distance_${segment}`)?.value ?? 60);
  ...
  summary.textContent = "Layout built. Enter designed/actual distances, baseline elevations, and cross-level elevations, then run the check.";
}

function collectValues(check) {
  return check.inputs.reduce((acc, input) => {
    const el = document.getElementById(`${check.id}_${input.id}`);
    acc[input.id] = Number(el.value);
    return acc;
  }, {});
}
...
function evaluateElevationRows() {
  ...
  for (let segment = 1; segment < columns; segment += 1) {
    ...
    offsets.forEach(offset => {
      const crossLevelValue = Math.abs(Number(document.getElementById(`cross_segment_${segment}_station_${offset}`)?.value ?? 0));
...
function exportPdfReport() {
  ...
  const suggestionHtml = suggestionList.innerHTML;
